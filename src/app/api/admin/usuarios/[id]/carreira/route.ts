...
function usuarioExiste(id: number): boolean {
  return !!getDb().prepare("SELECT id FROM usuarios WHERE id = ?").get(id);
}
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!usuarioExiste(usuarioId)) throw new ErroApi(404, "Usuário não encontrado.");

    const nivel = nivelAtual(usuarioId);
    ...
    return NextResponse.json({
      ...
      metricas: calcularMetricasSemana(usuarioId),
      metas: METAS_SEMANAIS,
      sinalizacao: avaliarSinalizacaoPromocao(usuarioId),
      historico: listarHistoricoNivel(usuarioId),
    });
  } catch (err) {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    definirNivel(usuarioId, body.nivel, body.observacao ?? "", admin.id);
